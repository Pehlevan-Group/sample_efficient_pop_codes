...
def k_expt(pvals, K, y, lamb, repeat = 100):
...
def target_fn_nonsmooth(X, beta):
    return np.exp(- ( beta * (np.abs(np.cos(X/2.0-math.pi/2.0))) )**(0.5)) - 0.2
...
y = target_fn_nonsmooth(X, beta)
norm = np.sqrt( np.mean(y**2) )
y = y / norm
plt.plot(X, y)
plt.show()

ys = target_fn_nonsmooth(x_s, beta) / norm
...
plt.plot(X, K1[0,:])
plt.scatter([],[], s = 1, color = 'C0', label ='narrow')
plt.plot(X, K2[0,:])
plt.scatter([],[], s=1, color = 'C1', label ='wide')
plt.plot(X, K3[0,:])
plt.scatter([],[], s=1, color = 'C2', label ='optimal')
plt.title('Different Bandwidths von-Mises')
plt.ylabel(r'$K(\theta)$', fontsize=myaxis_font)
plt.xlabel(r'$\theta$',fontsize=myaxis_font)
plt.ylim([-0.1, 1.1])
plt.legend(loc = 'upper right')
plt.tight_layout()
plt.savefig(fig_dir + 'wide_vs_narrow_kernel.pdf')
plt.show()
...
#plt.legend()
plt.tight_layout()
plt.savefig(fig_dir + 'wide_vs_narrow_spectra.pdf')
plt.show()
...
plt.xlabel(r'$\theta$', fontsize=myaxis_font)
plt.tight_layout()
plt.savefig(fig_dir+'bandwidth_vs_fit.pdf')
plt.show()
...
plt.tight_layout()
plt.savefig(fig_dir+ 'bandwidth_learning_curves.pdf')
plt.show()
...
plt.tight_layout()
plt.savefig(fig_dir+'pure_mode_task_bandwidth_contour.pdf')
plt.show()
...
plt.tight_layout()
plt.savefig(fig_dir+'theory_learning_curves.pdf')
plt.show()
...
cbar.ax.set_yticklabels([r'%0.1f'% np.amin(Eg), r'%0.1f'% np.amax(Eg)])
plt.tight_layout()
plt.savefig(fig_dir+'pure_mode_task_powerlaw_contour.pdf')
plt.show()
"""
