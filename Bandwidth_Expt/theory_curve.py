...
def fp(z, *args):
    p, spectrum, lamb = args
    return 1 - np.sum( spectrum/(p*spectrum+z) ) + z*np.sum(spectrum/(p*spectrum + z)**2)
...
def solve_implicit(pvals, spectrum, lamb):
    ...
    for i,p in enumerate(pvals):
        ...
        zvals[i] = sp.optimize.root_scalar(f = f, method = 'newton', fprime = fp, x0=(np.sum(spectrum)+lamb), x1= lamb, args = args).root
        #zvals[i] = sp.optimize.root_scalar(f=f, method = 'brentq', bracket = [lamb,np.sum(spectrum)+lamb], args = args).root
    return zvals
